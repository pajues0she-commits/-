...
- ✅ **자동 판정** — 판정 기준값(기본 −850 mV) 대비 합격/부적합 자동 표시 및 종합 판정
...
2. **① 판정 기준**에서 판정 기준값(mV)을 확인/수정합니다. (방식 방법은 외부전원법 고정)
3. **② 측정 지점**에서 지점을 추가하고, 위치명 · 측정 평균값(mV)을 입력한 뒤 측정지 사진을 촬영/첨부합니다.
...
- 판정 기준값(mV)은 **① 판정 기준**에서 변경할 수 있습니다. (방식 방법은 외부전원법으로 고정)
