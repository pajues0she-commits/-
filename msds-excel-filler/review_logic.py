...
CRITERIA_REVIEW = [
    ...
    "변경하고자 하는 경우, 또는 제조사가 변경된 경우 "
    "(단, CAS 번호·성분 함유량과 제조사가 모두 동일한 경우는 제외 — "
    "성분명 표기 방식 차이(예: 물/Water)는 CAS 번호 기준으로 같은 물질로 본다)",
...
CRITERIA_REGISTER = [
    ...
    "단, 기존 등록 물질과 CAS 번호·성분 함유량·제조사가 모두 동일하고 MSDS "
    "개정일자 변경이 없는 경우는 정보등록 대상이 아님 "
    "(MSDS 개정일자가 변경된 경우에는 정보등록 대상)",
]
...
def assess(name: str, components, db, consumer_product: bool = False,
           manufacturer: str = "", revision_date: str = "") -> dict:
    """도입검토·정보등록 대상 여부를 판독한다.

    db: 기존 등록 목록 [{"name", "manufacturer", "components", "revision", ...}]
    consumer_product: 일반 소매점에서 일반 소비자 대상으로 판매되는 물질 여부
    manufacturer / revision_date: 이번 MSDS의 제조사·개정일자.

    판정 규칙:
      · 성분 비교는 CAS 번호·함유량 기준 — 성분명 표기 차이(물/Water)는
        같은 물질로 본다.
      · CAS 번호·함유량·제조사가 모두 동일 → 도입검토·정보등록 대상 아님.
        단, MSDS 개정일자가 변경되었으면 정보등록 대상.
    ...
    if same_all:
        review, review_label = False, "도입검토 대상 아님"
        review_reason = ("기존 등록 물질과 CAS 번호·성분 함유량·제조사가 모두 "
                         "동일합니다(성분명 표기 차이는 무시) — 도입검토 "
                         "대상이 아닙니다.")
    elif same_comp:
        review, review_label = True, "도입검토 대상"
        review_reason = ("기존 등록 물질과 CAS 번호·성분 함유량은 동일하지만 "
                         "제조사가 변경되었습니다 — 도입검토 대상입니다.")
    elif same_name:
        review, review_label = True, "도입검토 대상"
    ...
    if consumer_product:
        ...
        prior_revs = [_norm(r.get("revision")) for r in same_all]
        if rev and all(pr and pr != rev for pr in prior_revs):
            register, register_label = True, "정보등록 대상"
            register_reason = ("CAS 번호·성분 함유량·제조사는 동일하지만 MSDS "
                               f"개정일자가 변경되었습니다(기존 "
                               f"{same_all[-1].get('revision') or '미상'} → "
                               f"이번 {revision_date}) — 정보등록 대상입니다.")
        else:
            register, register_label = False, "정보등록 대상 아님"
            register_reason = ("기존 등록과 CAS 번호·성분 함유량·제조사가 "
                               "동일하고 MSDS 개정일자 변경이 없습니다 — "
                               "정보등록 대상이 아닙니다.")
    else:
        register, register_label = True, "정보등록 대상"
